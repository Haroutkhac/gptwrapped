  ...
  const steps = [
    {
      ...
      description:
        "Open ChatGPT, click your profile icon, and navigate to Settings → Data Controls",
    },
    {
      number: 2,
      icon: Download,
      ...
      description:
        'Click "Export data" and confirm. You\'ll receive an email with a download link within a few minutes.',
    },
    {
      number: 3,
      icon: Upload,
      ...
      description:
        "Extract the ZIP file and drag the conversations.json file into the upload area below.",
    },
  ];
  ...
  return (
    <div className="mb-8">
      <h2 className="text-2xl font-bold text-white mb-6">
        How to Import Your ChatGPT Data
      </h2>
...
export default function ImportWorkflow() {
  ...
  const handleFiles = async (files: File[]) => {
    ...
    try {
      ...
      try {
        ...
        await clearRawConversations();
        setWarning(
          "Your conversation history is too large to store for AI topic analysis. Basic analytics will still work."
        );
      }
      ...
      const period = {
        start: result.summary.period.start,
        end: result.summary.period.end,
      };
      setRange(period);
  ...
  const handleRangeChange = (key: "start" | "end", value: string) => {
    ...
    setRange((prev) => {
      ...
      const filtered = filterConversationsByDate(
        conversations,
        next.start,
        next.end
      );
    ...
    <div className="max-w-4xl mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-[#B3B3B3]">
          Upload your ChatGPT export to see your personalized analytics and
          wrapped experience.
        </p>
      </div>
        ...
        {preview && (
          <article className="bg-[#181818] border border-[#282828] rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              <KpiStat
                label="Messages"
                value={preview.summary.totals.messages.toLocaleString()}
              />
              <KpiStat
                label="Words"
                value={preview.summary.totals.words.toLocaleString()}
              />
              <KpiStat
                label="Longest streak"
                value={`${preview.summary.longest_streak_days} days`}
              />
            </div>
            <div className="space-y-1 text-sm text-[#B3B3B3] mb-6">
              <p>
                Active days: {preview.summary.fun.active_days}/
                {preview.summary.fun.period_days} (
                {(
                  (preview.summary.fun.active_days /
                    preview.summary.fun.period_days) *
                    100 || 0
                ).toFixed(1)}
                %)
              ...
              <p>
                Word split:{" "}
                {(preview.summary.fun.user_words_pct * 100).toFixed(1)}% user /{" "}
                {(preview.summary.fun.assistant_words_pct * 100).toFixed(1)}%
                assistant
              </p>
              {preview.summary.fun.top_keyword && (
                <p>
                  Most used keyword: &ldquo;
                  {preview.summary.fun.top_keyword.term}&rdquo; (
                  {preview.summary.fun.top_keyword.count} mentions)
                </p>
              )}
            </div>

...
export interface TopicAnalysisResult {
  ...
  projections: { conversationId: string; x: number; y: number; clusterId: string }[];
}

const CLUSTER_COLORS = [
  '#8b5cf6', '#f472b6', '#22d3ee', '#facc15', '#34d399',
  '#f97316', '#2dd4bf', '#c084fc', '#fb7185', '#38bdf8',
  '#a3e635', '#fbbf24', '#e879f9', '#67e8f9', '#f43f5e'
];
...
function getConversationText(conv: NormalizedConversation, maxLength = 2000): string {
...
function extractKeywordsFromTitles(titles: string[], limit = 10): string[] {
  const stopwords = new Set([
    ...
    'than', 'too', 'very', 'just', 'about', 'help', 'make', 'using', 'use'
  ]);
...
function scoreSentiment(text: string): number {
  const lexicon: Record<string, number> = {
    love: 0.8, great: 0.6, awesome: 0.6, thanks: 0.4, good: 0.3,
    nice: 0.3, cool: 0.3, happy: 0.3, excited: 0.5, perfect: 0.5,
    excellent: 0.6, amazing: 0.6, wonderful: 0.5,
    bad: -0.4, broken: -0.5, bug: -0.4, error: -0.5, fail: -0.5,
    stuck: -0.3, terrible: -0.8, sad: -0.5, worry: -0.2, wrong: -0.3,
    issue: -0.3, problem: -0.4, fix: -0.2, crash: -0.6, hate: -0.8
  };
...
export async function analyzeTopicsWithEmbeddings(
  ...
  const conversationTexts = conversations.map(conv => getConversationText(conv));
  const batchSize = 100;
  ...
  const clusterEntries = Array.from(clusterMap.entries()).sort((a, b) => b[1].length - a[1].length);

...
export async function saveRawConversations(
...
export async function loadRawConversations(): Promise<
...
export function subscribeToStoredData(
  callback: (data: WrappedData | null) => void
) {
  if (typeof window === "undefined") return () => {};
...
export function saveTopicOverrides(topics: Topic[]) {
  ...
  window.dispatchEvent(
    new CustomEvent(TOPIC_OVERRIDES_EVENT, { detail: topics })
  );
}
...
export function clearTopicOverrides() {
  ...
  window.dispatchEvent(
    new CustomEvent(TOPIC_OVERRIDES_EVENT, { detail: null })
  );
}
...
export function saveEmbeddingAnalysis(analysis: TopicAnalysisResult) {
  ...
  window.localStorage.setItem(
    EMBEDDING_ANALYSIS_KEY,
    JSON.stringify(withoutEmbeddings)
  );
  window.dispatchEvent(
    new CustomEvent(EMBEDDING_ANALYSIS_EVENT, { detail: analysis })
  );
}
...
export function clearEmbeddingAnalysis() {
  ...
  window.dispatchEvent(
    new CustomEvent(EMBEDDING_ANALYSIS_EVENT, { detail: null })
  );
}

export function subscribeToEmbeddingAnalysis(
  callback: (analysis: TopicAnalysisResult | null) => void
) {
  if (typeof window === "undefined") return () => {};
  const handler = (event: Event) => {
  ...
  return () => {
    window.removeEventListener(
      EMBEDDING_ANALYSIS_EVENT,
      handler as EventListener
    );
  };
}

export function loadEmbeddingsEnabled(): boolean {
  if (typeof window === "undefined") return false;
  return window.localStorage.getItem(EMBEDDINGS_ENABLED_KEY) === "true";
}

export function saveEmbeddingsEnabled(enabled: boolean) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(
